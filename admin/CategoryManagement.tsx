    ...
    return (
        <div className="space-y-8">
            <div className="flex justify-between items-end">
                <div>
                    <h2 className="text-3xl font-serif font-bold text-stone-text dark:text-white">Menu Categories</h2>
                    <p className="text-stone-light dark:text-stone-500 mt-1">Organize your offerings into sections.</p>
                </div>
                <button
                    onClick={handleAdd}
                    className="bg-primary text-white px-6 py-3 rounded-2xl font-bold shadow-lg shadow-primary/20 hover:bg-opacity-90 transition-all flex items-center gap-2 active:scale-95"
                >
                    ...
                    <tbody className="divide-y divide-stone-50 dark:divide-stone-800/50">
                        {categories.map((category, index) => (
                            <tr key={category.id} className="group hover:bg-stone-50/40 dark:hover:bg-stone-800/20 transition-colors">
                                ...
                                <td className="px-8 py-6 font-medium text-stone-text dark:text-white">{category.label}</td>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-stone-900/60 backdrop-blur-md" onClick={() => setIsModalOpen(false)}></div>
                    <div className="relative bg-white dark:bg-stone-900 w-full max-w-lg rounded-[40px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
                        <div className="p-10">
                            <h3 className="text-2xl font-serif font-bold mb-8 text-stone-text dark:text-white">
                            ...
                            <form onSubmit={handleSave} className="space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-stone-50 dark:bg-stone-900/50 border-stone-100 dark:border-stone-800 rounded-2xl px-5 py-3.5 text-sm focus:ring-4 focus:ring-primary/10 transition-all outline-none disabled:opacity-50"
                                    />
                                ...
                                <div className="pt-4 flex gap-4">
                                    <button
                                        ...
                                        className="flex-1 px-6 py-4 rounded-2xl font-bold text-stone-light hover:bg-stone-50 dark:hover:bg-stone-800 transition-all"
                                    >
                                        Cancel
                                    </button>
                                    <button
                                        type="submit"
                                        disabled={saving}
                                        className="flex-1 bg-primary text-white px-6 py-4 rounded-2xl font-bold shadow-lg shadow-primary/20 hover:bg-opacity-90 transition-all disabled:opacity-50"
                                    >
